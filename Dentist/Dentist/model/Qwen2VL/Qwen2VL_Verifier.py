...
class Qwen2VL_Verifier(Verifier):
 
    def ask_model(self, image, prompt, use_image=True):
        if use_image and image is not None:
            return call_ollama("qwen2.5vl:7b", prompt, image)
        else:
            return call_ollama("qwen2.5vl:7b", prompt)
 
    def vqa_model_evaluation(self, original_image, questions):
        result = []
        for question in questions:
            answer = call_ollama("qwen2.5vl:7b", question, original_image)
